...
class AnimationController {
  constructor(canvas, ctx, dpr, size, lightMode = false) {
    this.canvas = canvas;
    this.ctx = ctx;
    this.dpr = dpr;
    this.size = size;
  ...
  render(p, controller) {
    ...
    if (q > 0) {
      ...
      if (displacementProgress < 0.3) {
        ...
        screenX =
          controller.lerp(baseX, targetX, midProgress) +
          perpX * midProgress;
        screenY =
          controller.lerp(baseY, targetY, midProgress) +
          perpY * midProgress;
      } else {
        ...
        const targetDistance =
          this.distance * this.expansionRate * 1.5;
      ...
      const vx =
        ((this.z - controller.cameraZ) * screenX) /
        controller.viewZoom;
      const vy =
        ((this.z - controller.cameraZ) * screenY) /
        controller.viewZoom;
...
export function SpiralAnimation({ lightMode = true }) {
